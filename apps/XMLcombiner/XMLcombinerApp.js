...
function traverseObject(obj, level, jsonElements, previousElement, multipleIndex) {
    for (var key in obj) {
        if (key == '$')continue;
        if (obj.hasOwnProperty(key)) {
            ...
            console.log(key);
            if(hasMultipleElements(jsonElements[level], key)){
                jsonElements[level].push({"element": key, "parentElement": previousElement, "multiple": true});
            }
            jsonElements[level].push({"element": key, "parentElement": previousElement, "multiple": false});

            traverse(obj[key], level + 1, jsonElements, key, multipleIndex);
        }
    }
...
function hasChilldren(x, y) {
    ...
    for (var i = 0; i < x.length; i++) {
        for (var j = 0; j < y.length; j++) {
            if (x[i].element == y[j].parentElement) {
                x[i] = null;
                break;
            }
...
function stepTwo(req, res, cb) {
    ...
    var elementData = getElementData(searchedJson);
    //console.log(elementData);

    res.render('XMLcombiner', {'step': 2, 'elementData': elementData, 'xml': builder.buildObject(json2Xml)});
}
